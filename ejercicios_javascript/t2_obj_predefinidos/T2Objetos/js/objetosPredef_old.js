...
do {
  ...
  let texto = prompt("Introduce dos textos separados por (,)");
  titulo = texto.substr(0, texto.lastIndexOf(","));
  barra = texto.substr(texto.lastIndexOf(",") + 1, texto.length);
} while (
  titulo.length < 3 ||
  titulo.length > 20 ||
  (barra !== "yes" && barra !== "no")
...
const abrirVentana = function abrirVentana() {
  open(
    "ventana.html",
    "abrirVentana",
...
/* window.onload = setTimeout(function abrirVentana() {
  open(
    `ventana.html,'abrirVentana','scrollbars=${barra},width=${w},height=${h}'`
  );
  /*   window.document.title = titol; */
/*}, 3000); */
